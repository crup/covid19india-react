...
function Table(props) {
  ...
  const sortByKey = (arr, key, isAscending) => {
    return arr.concat().sort((s1, s2) => {
      ...
      return (
        (v1 > v2 ? 1 : v1 == v2 && s1['state'] > s2['state'] ? 1 : -1) *
        (isAscending ? 1 : -1)
      );
    });
  };
  ...
  useEffect(() => {
    ...
    setSortedStates(
      [props.states[0]].concat(sortByKey(data, sortColumn, isAscending))
    );
  ...
  const tableRows = [
    {
      ...
      titleShort: 'State/UT',
      titleSmall: 'State/UT',
      desktopStyle: 'state-heading',
      mobileStyle: 'state-heading',
    },
    {
      key: 'confirmed',
      title: 'Confirmed',
      ...
      <table className="table fadeInUp" style={{animationDelay: '1s'}}>
        <thead>
          <tr>
            {tableRows.map(
              (
                {key, title, desktopStyle, mobileStyle, titleSmall, titleShort},
                index
              ) => (
                <th
                  key={index}
                ...
                >
                  <div className="heading-content">
                    <abbr
                      className={`${
                        window.innerWidth <= 769 ? mobileStyle : desktopStyle
                      }`}
                      title={title}
                    >
                    ...
                    <div
                      style={{
                        display:
                          sortData.sortColumn === key ? 'initial' : 'none',
                      }}
                    >
                      {sortData.isAscending ? (
                        <Icon.ArrowDown />
        ...
        <tbody>
          {sortedStates.length > 1 && props.summary === false && (
            <Row key={0} state={sortedStates[0]} total={true} />
          )}
        </tbody>
      </table>
    </>
